...
namespace HutongGames.PlayMaker.Actions
{
		...
		[Tooltip("Punch using Rect Transform from the target's current position to the given value. ")]
		[HelpUrl("http://dotween.demigiant.com/documentation.php")]
		public class DotweenUGuiRectTransformPunchTo : FsmStateAction
		{
			[ActionSection("Setup")]
			[RequiredField]
		...
		private string debugString = "<b>[DotweenUGuiRectTransformPunchTo]</b><color=#E9E581ff> ...mmm did 'DoTween' work ?? </color>";
		...
		public override void Reset ()
		{
				...
				set2DTo = new FsmVector2 { UseVariable = true };
				setDelay = 0f;
				isSpeedBased = false;
				settingLoops = 0;
			...
			{
			if (duration.Value == 0) {
				Debug.LogWarning ("<b>[DotweenUGuiRectTransformPunchTo]</b><color=#E9E581ff>!!! Duration cannot be 0 !!!  </color>",this.Owner);
				Finish();
			}
			...
			case loopType.None:
				if (settingLoops > 0 || settingLoops < 0) Debug.LogWarning ("<b>[DotweenUGuiRectTransformPunchTo]</b><color=#FF9900ff>!!! Loop Time is set but no 'Loop Type' is selected !!! </color>",this.Owner);
				break;
			...
			_target.GetComponent<RectTransform>().DOPunchAnchorPos(Vector2setup, duration.Value,vibrato.Value, elasticity.Value,snapping.Value ).SetAs(setFinal);
