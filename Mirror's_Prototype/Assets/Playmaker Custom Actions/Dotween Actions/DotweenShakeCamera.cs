...
namespace HutongGames.PlayMaker.Actions
{
		...
		[Tooltip("Shake Camera from the target's current position to the given value. ")]
		[HelpUrl("http://dotween.demigiant.com/documentation.php")]
		public class DotweenShakeCamera : FsmStateAction
		{
			[ActionSection("Setup")]
			[RequiredField]
			...
			[TitleAttribute("Randomness (0 - Max 180)")]
			public FsmFloat elasticity;


			[ActionSection("Vector 3 Setup")]
			public FsmGameObject toObject;
			[Tooltip("Vector 3 Setup. NOTE: You can override individual axis below.")]
			[UIHint(UIHint.FsmVector3)]
			[TitleAttribute("Shake Direction")]
			...
			public FsmFloat z;
			[UIHint(UIHint.FsmFloat)]
			[Tooltip("Passing a float instead of a Vector3 allows to scale shake uniformly. NOTE: Will override Vector 3.")]
			public FsmFloat shakeBy;
		...
		[UIHint(UIHint.FsmBool)]
		[Tooltip("If TRUE the tween fade out.")]	
		...
		private string debugString = "<b>[DotweenShakeCamera]</b><color=#E9E581ff> ...mmm did 'DoTween' work ?? </color>";
		...
		public override void Reset ()
		{
				...
				shakeBy = new FsmFloat { UseVariable = true };
			...
			if (duration.Value == 0) {
				Debug.LogWarning ("<b>[DotweenShakeCamera]</b><color=#E9E581ff>!!! Duration cannot be 0 !!! </color>",this.Owner);
				Finish();
			}
			...
			if(elasticity.Value >180f) elasticity.Value = 180f;
			if(elasticity.Value <0f) elasticity.Value = 0f;
			...
			case loopType.None:
				if (settingLoops > 0 || settingLoops < 0) Debug.LogWarning ("<b>[DotweenShakeCamera]</b><color=#FF9900ff>!!! Loop Time is set but no 'Loop Type' is selected !!! </color>",this.Owner);
				break;
			...
			switch(dotweenTypeSelect){
				...
				if (shakeBy.IsNone)
					_target.GetComponent<Camera>().DOShakePosition(duration.Value,to.Value,vibrato.Value, elasticity.Value,fadeOut.Value).SetAs(setFinal);
					else
					_target.GetComponent<Camera>().DOShakePosition(duration.Value,shakeBy.Value,vibrato.Value, elasticity.Value,fadeOut.Value).SetAs(setFinal);
				break;
					
				case doTweenType.DoShakeRotation:
				if (shakeBy.IsNone)
					_target.GetComponent<Camera>().DOShakeRotation(duration.Value,to.Value,vibrato.Value, elasticity.Value,fadeOut.Value ).SetAs(setFinal);
					else
					_target.GetComponent<Camera>().DOShakeRotation(duration.Value,shakeBy.Value,vibrato.Value, elasticity.Value,fadeOut.Value ).SetAs(setFinal);
					break;
