...
namespace HutongGames.PlayMaker.Actions
{
		...
		[Tooltip("Change target's fill amount (0 to 1). ")]
		[HelpUrl("http://dotween.demigiant.com/documentation.php")]
		public class DotweenUGuiImgFillAmountTo : FsmStateAction
		{
			[ActionSection("Setup")]
			[RequiredField]
			...
			[ActionSection("Fill Setup")]
			[UIHint(UIHint.FsmFloat)]
			[Tooltip("Min 0 - Max 1")]	
			[TitleAttribute("Fill to")]
			public FsmFloat endTo;
		...
		private string debugString = "<b>[DotweenUGuiImgFillAmountTo]</b><color=#E9E581ff> ...mmm did 'DoTween' work ?? </color>";
		...
		public override void Reset ()
		{
			...
			if (endTo.Value < 0 || endTo.Value > 1){
				Debug.LogWarning ("<b>[DotweenUGuiImgFillAmountTo]</b><color=#FF9900ff>!!! Cannot be less than 0 (min) or above 1(max) !!! </color>",this.Owner);
			}

			switch(loopTypeSelect){
			case loopType.None:
				if (settingLoops > 0 || settingLoops < 0) Debug.LogWarning ("<b>[DotweenUGuiImgFillAmountTo]</b><color=#FF9900ff>!!! Loop Time is set but no 'Loop Type' is selected !!! </color>",this.Owner);
				break;
			...
			case doTweenType.Image:
				_target.GetComponent<UnityEngine.UI.Image>().DOFillAmount(endTo.Value,duration.Value).SetAs(setFinal);
				break;
