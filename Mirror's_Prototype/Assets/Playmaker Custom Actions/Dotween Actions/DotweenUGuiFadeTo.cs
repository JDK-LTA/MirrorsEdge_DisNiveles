...
namespace HutongGames.PlayMaker.Actions
{
		...
		[Tooltip("Fade using the renderer or audio from the target's current position to the given value. ")]
		[HelpUrl("http://dotween.demigiant.com/documentation.php")]
		public class DotweenUGuiFadeTo : FsmStateAction
		{
			[ActionSection("Setup")]
			[RequiredField]
			...
			[Tooltip("Alpha or Volume NOTE: Material works only with materials that support alpha")]	
		...
		private string debugString = "<b>[DotweenUGuiFadeTo]</b><color=#E9E581ff> ...mmm did 'DoTween' work ?? </color>";
		...
		public override void Reset ()
		{
			...
			case loopType.None:
				if (settingLoops > 0 || settingLoops < 0) Debug.LogWarning ("<b>[DotweenUGuiFadeTo]</b><color=#FF9900ff>!!! Loop Time is set but no 'Loop Type' is selected !!! </color>",this.Owner);
				break;
			...
			case doTweenType.CanvasGroup:
				_target.GetComponent<CanvasGroup>().DOFade(fadeTo.Value, duration.Value).SetAs(setFinal);
				break;
				
			case doTweenType.Graphic:
				_target.GetComponent<UnityEngine.UI.Graphic>().DOFade(fadeTo.Value,duration.Value).SetAs(setFinal);
				break;
				
			case doTweenType.Image:
				_target.GetComponent<UnityEngine.UI.Image>().DOFade(fadeTo.Value,duration.Value).SetAs(setFinal);
				break;

			case doTweenType.Outline:
				_target.GetComponent<UnityEngine.UI.Outline>().DOFade(fadeTo.Value,duration.Value).SetAs(setFinal);
				break;

			case doTweenType.Text:
				_target.GetComponent<UnityEngine.UI.Text>().DOFade(fadeTo.Value,duration.Value).SetAs(setFinal);
				break;
