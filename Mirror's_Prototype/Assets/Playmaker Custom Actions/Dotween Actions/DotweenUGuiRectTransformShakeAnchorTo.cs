...
namespace HutongGames.PlayMaker.Actions
{
		...
		[Tooltip("Shake using anchoredPosition from the target's current position to the given value. ")]
		[HelpUrl("http://dotween.demigiant.com/documentation.php")]
		public class DotweenUGuiRectTransformShakeAnchorTo : FsmStateAction
		{
			[ActionSection("Setup")]
			[RequiredField]
		...
		private string debugString = "<b>[DotweeDotweenUGuiRectTransformShakeAnchorTonCamera]</b><color=#E9E581ff> ...mmm did 'DoTween' work ?? </color>";
		...
		public override void Reset ()
		{
			...
			case loopType.None:
				if (settingLoops > 0 || settingLoops < 0) Debug.LogWarning ("<b>[DotweenUGuiRectTransformShakeAnchorTo]</b><color=#FF9900ff>!!! Loop Time is set but no 'Loop Type' is selected !!! </color>",this.Owner);
				break;
			...
			// Easy part to edit for other DotTween actions --->
				...
				if (shakeBy.IsNone)
					_target.GetComponent<RectTransform>().DOShakeAnchorPos(duration.Value,to.Value,vibrato.Value, elasticity.Value,snapping.Value ).SetAs(setFinal);
				else
					_target.GetComponent<RectTransform>().DOShakeAnchorPos(duration.Value,shakeBy.Value,vibrato.Value, elasticity.Value,snapping.Value ).SetAs(setFinal);
