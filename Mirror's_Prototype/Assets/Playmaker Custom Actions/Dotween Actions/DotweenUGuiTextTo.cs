...
namespace HutongGames.PlayMaker.Actions
{
		...
		public class DotweenUGuiTextTo : FsmStateAction
		{
			[ActionSection("Setup")]
			[RequiredField]
			public FsmOwnerDefault gameObject;

			[ActionSection("Text Setup")]
			[UIHint(UIHint.FsmString)]
			[TitleAttribute("Set text to")]	
		...
		private string debugString = "<b>[DotweenUGuiTextTo]</b><color=#E9E581ff> ...mmm did 'DoTween' work ?? </color>";
		...
		public override void Reset ()
		{
			...
			if (scrambleTypeSelect != scrambleType.None && scrambleChars.Value.Length > 0 && scrambleChars.Value.Length <10) Debug.LogWarning ("<b>[DotweenUGuiTextTo]</b><color=#FF9900ff!!!  Use as many characters as possible (minimum 10) in 'Scramble Chars' because DOTween uses a fast scramble mode which gives better results with more characters. !!! </color>",this.Owner);

			switch(loopTypeSelect){
			case loopType.None:
				if (settingLoops > 0 || settingLoops < 0) Debug.LogWarning ("<b>[DotweenUGuiTextTo]</b><color=#FF9900ff>!!! Loop Time is set but no 'Loop Type' is selected !!! </color>",this.Owner);
				break;
			...
			_target.GetComponent<UnityEngine.UI.Text>().DOText(text.Value,duration.Value,richTextEnabled.Value, scrambleTypeSelected,scrambleChars.Value).SetAs(setFinal);
