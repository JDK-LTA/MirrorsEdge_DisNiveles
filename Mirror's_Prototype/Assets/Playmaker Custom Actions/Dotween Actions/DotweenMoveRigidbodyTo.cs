...
namespace HutongGames.PlayMaker.Actions
{
	...
	[Tooltip("Moves using rigidbody from the target's current position to the given value. MovePosition/MoveRotation methods in the background, to correctly animate things related to physics objects. ")]
		[HelpUrl("http://dotween.demigiant.com/documentation.php")]
		public class DotweenMoveRigidbodyTo : FsmStateAction
		{
			[ActionSection("Setup")]
			[RequiredField]
		...
		private string debugString = "<b>[DotweenMoveRigidbodyTo]</b><color=#E9E581ff> ...mmm did 'DoTween' work ?? </color>";
		...
		public override void Reset ()
		{
			...
			case loopType.None:
				if (settingLoops > 0 || settingLoops < 0) Debug.LogWarning ("<b>[DotweenMoveRigidbodyTo]</b><color=#FF9900ff>!!! Loop Time is set but no 'Loop Type' is selected !!! </color>",this.Owner);
				break;
			...
			switch(dotweenTypeSelect){
				...
				_target.GetComponent<Rigidbody>().DOMove(to.Value, duration.Value,snapping.Value ).SetAs(setFinal);
				break;
					
				case doTweenType.DoMoveX:
					float x = to.Value.x;
				_target.GetComponent<Rigidbody>().DOMoveX(x, duration.Value,snapping.Value ).SetAs(setFinal);
					break;
					
				case doTweenType.DoMoveY:
					float y = to.Value.y;
				_target.GetComponent<Rigidbody>().DOMoveY(y, duration.Value,snapping.Value ).SetAs(setFinal);
					break;
					
				case doTweenType.DoMoveZ:
					float z = to.Value.z;
				_target.GetComponent<Rigidbody>().DOMoveZ(z, duration.Value,snapping.Value ).SetAs(setFinal);
					break;
