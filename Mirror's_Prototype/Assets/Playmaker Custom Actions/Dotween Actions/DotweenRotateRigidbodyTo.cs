...
namespace HutongGames.PlayMaker.Actions
{
		...
		[Tooltip("Rotate using rigidbody from the target's current position to the given value. ")]
		[HelpUrl("http://dotween.demigiant.com/documentation.php")]
	public class DotweenRotateRigidbodyTo : FsmStateAction
		{
			[ActionSection("Setup")]
			[RequiredField]
		...
		private string debugString = "<b>[DotweenRotateRigidbodyTo]</b><color=#E9E581ff> ...mmm did 'DoTween' work ?? </color>";
		...
		public override void Reset ()
		{
			...
			case loopType.None:
				if (settingLoops > 0 || settingLoops < 0) Debug.LogWarning ("<b>[DotweenRotateRigidbodyTo]</b><color=#FF9900ff>!!! Loop Time is set but no 'Loop Type' is selected !!! </color>",this.Owner);
				break;
			...
			// Easy part to edit for other DotTween actions --->
				...
				_target.GetComponent<Rigidbody>().DORotate(to.Value, duration.Value, RotateMode.Fast ).SetAs(setFinal);
					break;
				case RotationTypeSelect.FastBeyond360:
				_target.GetComponent<Rigidbody>().DORotate(to.Value, duration.Value, RotateMode.FastBeyond360 ).SetAs(setFinal);
					break;
				case RotationTypeSelect.LocalAxisAdd:
				_target.GetComponent<Rigidbody>().DORotate(to.Value, duration.Value, RotateMode.LocalAxisAdd ).SetAs(setFinal);
					break;
				case RotationTypeSelect.WorldAxisAdd:
				_target.GetComponent<Rigidbody>().DORotate(to.Value, duration.Value, RotateMode.WorldAxisAdd ).SetAs(setFinal);
					break;
				}
