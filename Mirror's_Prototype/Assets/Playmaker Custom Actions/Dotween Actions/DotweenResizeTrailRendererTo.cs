...
namespace HutongGames.PlayMaker.Actions
{
		...
		[Tooltip("Changes the TrailRenderer's startWidth/endWidth to the given ones")]
		[HelpUrl("http://dotween.demigiant.com/documentation.php")]
		public class DotweenResizeTrailRendererTo : FsmStateAction
		{
			[ActionSection("Setup")]
			[RequiredField]
			...
			[ActionSection("TrailRenderer Setup")]
		...
		private string debugString = "<b>[DotweenResizeTrailRendererTo]</b><color=#E9E581ff> ...mmm did 'DoTween' work ?? </color>";
		...
		public override void Reset ()
		{
			...
			trailRenderer = _target.GetComponent<TrailRenderer>();

			if (trailRenderer == null)
			{
				Debug.LogWarning("<b>[DotweenResizeTrailRendererTo]</b><color=#FF9900ff> Missing TrailRenderer component </color>",this.Owner);
				return;
			}	
			...
			case loopType.None:
						if (settingLoops > 0 || settingLoops < 0) Debug.LogWarning ("<b>[DotweenResizeTrailRendererTo]</b><color=#FF9900ff>!!! Loop Time is set but no 'Loop Type' is selected !!! </color>",this.Owner);
				break;
			...
			trailRenderer.DOResize(startWidth.Value, endWidth.Value, duration.Value).SetAs(setFinal);
