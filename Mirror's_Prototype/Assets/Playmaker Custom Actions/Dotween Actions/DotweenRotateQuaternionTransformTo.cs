...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DotweenRotateQuaternionTransformTo : FsmStateAction
		{
			[ActionSection("Setup")]
			[RequiredField]
			...
			[ActionSection("Quaternion Setup")]
		...
		private string debugString = "<b>[DotweenRotateQuaternionTransformTo]</b><color=#E9E581ff> ...mmm did 'DoTween' work ?? </color>";
		...
		public override void Reset ()
		{
			...
			case loopType.None:
				if (settingLoops > 0 || settingLoops < 0) Debug.LogWarning ("<b>[DotweenRotateQuaternionTransformTo]</b><color=#FF9900ff>!!! Loop Time is set but no 'Loop Type' is selected !!! </color>",this.Owner);
				break;
			...
			if (dotweenTypeSelect == doTweenType.DoRotate){
					
				_target.transform.DORotateQuaternion(toQuaternion.Value, duration.Value).SetAs(setFinal);
			
				}

			else {

				_target.transform.DOLocalRotateQuaternion(toQuaternion.Value, duration.Value).SetAs(setFinal);
