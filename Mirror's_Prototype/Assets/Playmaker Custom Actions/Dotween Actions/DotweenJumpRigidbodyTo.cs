...
namespace HutongGames.PlayMaker.Actions
{
		...
		[Tooltip("Jump using Rigidbody from the target's current position to the given value. ")]
		[HelpUrl("http://dotween.demigiant.com/documentation.php")]
		public class DotweenJumpRigidbodyTo : FsmStateAction
		{
			[ActionSection("Setup")]
			[RequiredField]
		...
		private string debugString = "<b>[DotweenJumpRigidbodyTo]</b><color=#E9E581ff> ...mmm did 'DoTween' work ?? </color>";
		...
		public override void Reset ()
		{
			...
			if (rigidBody == null)
			{
				Debug.LogWarning ("<b>[DotweenJumpRigidbodyTo]</b><color=#FF9900ff>Missing Rigidbody component</color>",this.Owner);
				return;
			}	
			...
			case loopType.None:
				if (settingLoops > 0 || settingLoops < 0) Debug.LogWarning ("<b>[DotweenJumpRigidbodyTo]</b><color=#FF9900ff>!!! Loop Time is set but no 'Loop Type' is selected !!! </color>",this.Owner);
				break;
