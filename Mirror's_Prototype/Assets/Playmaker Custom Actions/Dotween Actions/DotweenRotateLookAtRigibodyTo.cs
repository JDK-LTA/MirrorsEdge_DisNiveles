...
namespace HutongGames.PlayMaker.Actions
{
		...
		[Tooltip("Rotate Look At using rigibody from the target's current position to the given value. ")]
		[HelpUrl("http://dotween.demigiant.com/documentation.php")]
		public class DotweenRotateLookAtRigibodyTo : FsmStateAction
		{
			[ActionSection("Setup")]
			[RequiredField]
		...
		public enum AxisConstraintSelect

		{
			...
			public AxisConstraintSelect axisConstraintSelect;

			[Tooltip("The vector that defines in which direction up is. Default: Vector3.up / Vector3(0, 1, 0).")]
			public FsmVector3 UpDirection;

			[ActionSection("Vector 3 Look At Setup")]
			[TitleAttribute("Look At Object")]
			public FsmGameObject lookat;
			[Tooltip("Vector3 Setup. NOTE: Look At Gameobject override Vector 3 but you can override individual axis below.")]
			[TitleAttribute("or set Look At Vector 3")]
		...
		AxisConstraint axisData;


		private string debugString = "<b>[DotweenRotateLookAtRigibodyTo]</b><color=#E9E581ff> ...mmm did 'DoTween' work ?? </color>";
		...
		public override void Reset ()
		{
				...
				lookat = new FsmGameObject { UseVariable = true };
				setDelay = 0f;
				isSpeedBased = false;
				settingLoops = 0;
				...
				startEvent = null;
				finishEvent = null;
				UpDirection = new FsmVector3 { UseVariable = true };
			...
			if (UpDirection.IsNone == true) UpDirection.Value = new Vector3(0, 1, 0);
			...
			case loopType.None:
				if (settingLoops > 0 || settingLoops < 0) Debug.LogWarning ("<b>[DotweenRotateLookAtRigibodyTo]</b><color=#FF9900ff>!!! Loop Time is set but no 'Loop Type' is selected !!! </color>",this.Owner);
				break;
			...
			_target.GetComponent<Rigidbody>().DOLookAt(to.Value, duration.Value, axisData, UpDirection.Value ).SetAs(setFinal);
