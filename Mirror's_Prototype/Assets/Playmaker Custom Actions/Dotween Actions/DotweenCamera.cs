...
namespace HutongGames.PlayMaker.Actions
{
		...
		[Tooltip("Tweens a Camera's doAspect / farClipPlane / fieldOfView / nearClipPlane / orthographicSize ")]
		[HelpUrl("http://dotween.demigiant.com/documentation.php")]
		public class DotweenCamera : FsmStateAction
		{
			[ActionSection("Setup")]
		...
		private string debugString = "<b>[DotweenCamera]</b><color=#E9E581ff> ...mmm did 'DoTween' work ?? </color>";
		...
		public override void Reset ()
		{
			...
			case loopType.None:
				if (settingLoops > 0 || settingLoops < 0) Debug.LogWarning ("<b>[DotweenCamera]</b><color=#FF9900ff>!!! Loop Time is set but no 'Loop Type' is selected !!! </color>",this.Owner);
				break;
			...
			case doTweenType.doAspect:
				_target.GetComponent<Camera>().DOAspect(to.Value, duration.Value).SetAs(setFinal);
				break;
				
			case doTweenType.farClipPlane:
				_target.GetComponent<Camera>().DOFarClipPlane(to.Value, duration.Value).SetAs(setFinal);
				break;

			case doTweenType.fieldOfView:
				_target.GetComponent<Camera>().DOFieldOfView(to.Value, duration.Value).SetAs(setFinal);
				break;
				          
			case doTweenType.nearClipPlane:
				_target.GetComponent<Camera>().DONearClipPlane(to.Value, duration.Value).SetAs(setFinal);
				break;

			case doTweenType.orthographicSize:
				_target.GetComponent<Camera>().DOOrthoSize(to.Value, duration.Value).SetAs(setFinal);
				break;
			}
