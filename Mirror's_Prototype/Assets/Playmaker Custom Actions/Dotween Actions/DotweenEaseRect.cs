...
namespace HutongGames.PlayMaker.Actions
{
		...
		public class DotweenEaseRect : FsmStateAction
		{
		...
		private string debugString = "<b>[DotweenEaseRect]</b><color=#E9E581ff> ...mmm did 'DoTween' work ?? </color>";
			
		public override void Reset ()
		{
				toValue = new FsmRect { UseVariable = true };
				fromValue = new FsmRect { UseVariable = true };
				resultValue = new FsmRect { UseVariable = true };
				duration = 0f;
				setDelay = 0f;
				isSpeedBased = false;
				settingLoops = 0;
			...
			case loopType.None:
				if (settingLoops > 0 || settingLoops < 0) Debug.LogWarning ("<b>[DotweenEaseRect]</b><color=#FF9900ff>!!! Loop Time is set but no 'Loop Type' is selected !!! </color>",this.Owner);
				break;
			...
			DOTween.To(()=>fromValue.Value, x=>resultValue.Value = x, toValue.Value,duration.Value).SetOptions(snapping.Value).SetAs(setFinal);
