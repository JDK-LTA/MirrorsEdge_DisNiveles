...
namespace HutongGames.PlayMaker.Actions
{
		...
		[Tooltip("Scale using Transform from the target's current position to the given value. ")]
		[HelpUrl("http://dotween.demigiant.com/documentation.php")]
		public class DotweenScaleTransformTo : FsmStateAction
		{
			[ActionSection("Setup")]
			[RequiredField]
			...
			public FsmFloat z;
			[UIHint(UIHint.FsmFloat)]
			[Tooltip("Only use with DoScale. Passing a float instead of a Vector3 allows to scale stuff uniformly. NOTE: Will override Vector 3.")]
			public FsmFloat scaleBy;
		...
		private string debugString = "<b>[DotweenScaleTransformTo]</b><color=#E9E581ff> ...mmm did 'DoTween' work ?? </color>";
		...
		public override void Reset ()
		{
				...
				scaleBy = new FsmFloat { UseVariable = true };
			...
			setFinal = new TweenParams()
				.SetDelay(setDelay.Value)
				.SetAutoKill(setAutoKill.Value)
				.SetSpeedBased(isSpeedBased.Value)
				.SetRelative(setRelative.Value)
				.OnComplete(MyCallback);


			if(dotweenTypeSelect != doTweenType.DoScale && !scaleBy.IsNone) Debug.LogWarning("<b>[DotweenScaleTransformTo]</b><color=#FF9900ff> Scale By only works with DoScale select </color>",this.Owner);
			...
			case loopType.None:
				if (settingLoops > 0 || settingLoops < 0) Debug.LogWarning ("<b>[DotweenScaleTransformTo]</b><color=#FF9900ff>!!! Loop Time is set but no 'Loop Type' is selected !!! </color>",this.Owner);
				break;
			...
			switch(dotweenTypeSelect){
				...
				if (scaleBy.IsNone)
				_target.transform.DOScale(to.Value, duration.Value).SetAs(setFinal);
				else 
					_target.transform.DOScale(scaleBy.Value, duration.Value).SetAs(setFinal);
				break;
					
			case doTweenType.DoScaleX:
					float x = to.Value.x;
				_target.transform.DOScaleX(x, duration.Value).SetAs(setFinal);
					break;
					
			case doTweenType.DoScaleY:
					float y = to.Value.y;
				_target.transform.DOScaleY(y, duration.Value).SetAs(setFinal);
					break;
					
			case doTweenType.DoScaleZ:
					float z = to.Value.z;
				_target.transform.DOScaleZ(z, duration.Value).SetAs(setFinal);
					break;
