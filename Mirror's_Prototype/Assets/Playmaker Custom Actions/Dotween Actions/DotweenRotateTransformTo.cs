...
namespace HutongGames.PlayMaker.Actions
{
		...
		[Tooltip("Rotate using Transform from the target's current position to the given value. ")]
		[HelpUrl("http://dotween.demigiant.com/documentation.php")]
	public class DotweenRotateTransformTo : FsmStateAction
		{
			[ActionSection("Setup")]
			[RequiredField]
		...
		public enum RotationTypeSelect  {
		...
		public RotationTypeSelect rotationType;
		...
		private string debugString = "<b>[DotweenRotateTransformTo]</b><color=#E9E581ff> ...mmm did 'DoTween' work ?? </color>";
		...
		public override void Reset ()
		{
			...
			case loopType.None:
				if (settingLoops > 0 || settingLoops < 0) Debug.LogWarning ("<b>[DotweenRotateTransformTo]</b><color=#FF9900ff>!!! Loop Time is set but no 'Loop Type' is selected !!! </color>",this.Owner);
				break;
			...
			case doTweenType.DoRotate:
				...
				case RotationTypeSelect.Fast:
				_target.transform.DORotate(to.Value, duration.Value, RotateMode.Fast ).SetAs(setFinal);
					break;
				case RotationTypeSelect.FastBeyond360:
					_target.transform.DORotate(to.Value, duration.Value, RotateMode.FastBeyond360 ).SetAs(setFinal);
					break;
				case RotationTypeSelect.LocalAxisAdd:
					_target.transform.DORotate(to.Value, duration.Value, RotateMode.LocalAxisAdd ).SetAs(setFinal);
					break;
				case RotationTypeSelect.WorldAxisAdd:
					_target.transform.DORotate(to.Value, duration.Value, RotateMode.WorldAxisAdd ).SetAs(setFinal);
					break;
				}
			...
			case doTweenType.DoLocalRotate:
				...
				case RotationTypeSelect.Fast:
					_target.transform.DOLocalRotate(to.Value, duration.Value, RotateMode.Fast ).SetAs(setFinal);
					break;
				case RotationTypeSelect.FastBeyond360:
					_target.transform.DOLocalRotate(to.Value, duration.Value, RotateMode.FastBeyond360 ).SetAs(setFinal);
					break;
				case RotationTypeSelect.LocalAxisAdd:
					_target.transform.DOLocalRotate(to.Value, duration.Value, RotateMode.LocalAxisAdd ).SetAs(setFinal);
					break;
				case RotationTypeSelect.WorldAxisAdd:
					_target.transform.DOLocalRotate(to.Value, duration.Value, RotateMode.WorldAxisAdd ).SetAs(setFinal);
					break;
				}
