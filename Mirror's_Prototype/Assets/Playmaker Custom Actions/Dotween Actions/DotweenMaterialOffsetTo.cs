...
namespace HutongGames.PlayMaker.Actions
{
		...
		[Tooltip("Tween the target's textureOffset to the given one. ")]
		[HelpUrl("http://dotween.demigiant.com/documentation.php")]
		public class DotweenMaterialOffsetTo : FsmStateAction
		{
		[ActionSection("Setup")]
		...
		[ActionSection("Material Name (optional)")]
		...
		private string debugString = "<b>[DotweenMaterialOffsetTo]</b><color=#E9E581ff> ...mmm did 'DoTween' work ?? </color>";
		...
		public override void Reset ()
		{
			...
			case loopType.None:
				if (settingLoops > 0 || settingLoops < 0) Debug.LogWarning ("<b>[DotweenMaterialOffsetTo]</b><color=#FF9900ff>!!! Loop Time is set but no 'Loop Type' is selected !!! </color>",this.Owner);
				break;
			...
			// Easy part to edit for other DotTween actions --->
				...
				_target.GetComponent<Renderer>().material.DOOffset(to.Value,materialName.Value,duration.Value).SetAs(setFinal);
				else
				_target.GetComponent<Renderer>().material.DOOffset(to.Value,duration.Value).SetAs(setFinal);
