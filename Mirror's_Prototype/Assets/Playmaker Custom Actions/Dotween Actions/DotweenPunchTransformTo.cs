...
namespace HutongGames.PlayMaker.Actions
{
		...
		[Tooltip("Punch using Transform from the target's current position to the given value. ")]
		[HelpUrl("http://dotween.demigiant.com/documentation.php")]
		public class DotweenPunchTransformTo : FsmStateAction
		{
			[ActionSection("Setup")]
			[RequiredField]
			...
			[TitleAttribute("Elasticity (Min 0 - Max 1)")]
			...
			[TitleAttribute("Punch Direction")]
		...
		private string debugString = "<b>[DotweenPunchTransformTo]</b><color=#E9E581ff> ...mmm did 'DoTween' work ?? </color>";
		...
		public override void Reset ()
		{
				...
				to = new FsmVector3 { UseVariable = true };
			...
			{
			if (duration.Value == 0) {
				Debug.LogWarning ("<b>[DotweenPunchTransformTo]</b><color=#FF9900ff>!!! Duration cannot be 0 !!! </color>",this.Owner);
				Finish();
			}
			...
			if(elasticity.Value >1f) elasticity.Value = 1;
			if(elasticity.Value <0f) elasticity.Value = 0;
			...
			case loopType.None:
				if (settingLoops > 0 || settingLoops < 0) Debug.LogWarning ("<b>[DotweenPunchTransformTo]</b><color=#FF9900ff>!!! Loop Time is set but no 'Loop Type' is selected !!! </color>\",this.Owner");
				break;
			...
			switch(dotweenTypeSelect){
				...
				case doTweenType.DoPunchPosition:
				_target.transform.DOPunchPosition(to.Value, duration.Value,vibrato.Value, elasticity.Value,snapping.Value ).SetAs(setFinal);
				break;
					
				case doTweenType.DoPunchRotation:
				_target.transform.DOPunchRotation(to.Value, duration.Value,vibrato.Value, elasticity.Value ).SetAs(setFinal);
					break;
					
				case doTweenType.DoPunchScale:
				_target.transform.DOPunchScale(to.Value, duration.Value,vibrato.Value, elasticity.Value ).SetAs(setFinal);
					break;
