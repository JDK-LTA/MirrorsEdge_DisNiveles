...
namespace HutongGames.PlayMaker.Actions
{
		...
		public class DotweenEaseColor : FsmStateAction
		{
		...
		private string debugString = "<b>[DotweenEaseColor]</b><color=#E9E581ff> ...mmm did 'DoTween' work ?? </color>";
		...
		public override void Reset ()
		{
			...
			case loopType.None:
				if (settingLoops > 0 || settingLoops < 0) Debug.LogWarning ("<b>[DotweenEaseColor]</b><color=#FF9900ff>!!! Loop Time is set but no 'Loop Type' is selected !!! </color>",this.Owner);
				break;
			...
			DOTween.To(()=>fromValue.Value, x=>resultValue.Value = x, toValue.Value,duration.Value).SetOptions(onlyAlpha.Value).SetAs(setFinal);
