...
namespace HutongGames.PlayMaker.Actions
{
		...
		[Tooltip("Tweens the target's min or max anchoredPosition")]
		[HelpUrl("http://dotween.demigiant.com/documentation.php")]
		public class DotweenUGuiRectTransformAnchorMinMaxTo : FsmStateAction
		{
			[ActionSection("Setup")]
			[RequiredField]
		...
		private string debugString = "<b>[DotweenUGuiRectTransformAnchorMinMaxTo]</b><color=#E9E581ff> ...mmm did 'DoTween' work ?? </color>";
		...
		public override void Reset ()
		{
			...
			case loopType.None:
				if (settingLoops > 0 || settingLoops < 0) Debug.LogWarning ("<b>[DotweenUGuiRectTransformAnchorMinMaxTo]</b><color=#FF9900ff>!!! Loop Time is set but no 'Loop Type' is selected !!! </color>",this.Owner);
				break;
			...
			if (dotweenTypeSelect == doTweenType.AnchoredMin)
				_target.GetComponent<RectTransform>().DOAnchorMin(Vector2setup,duration.Value, snapping.Value).SetAs(setFinal);

				
		else
				_target.GetComponent<RectTransform>().DOAnchorMax(Vector2setup,duration.Value, snapping.Value).SetAs(setFinal);
