...
namespace HutongGames.PlayMaker.Actions
{
		...
		[Tooltip("Tween Color to the renderer. ")]
		[HelpUrl("http://dotween.demigiant.com/documentation.php")]
		public class DotweenColorRendererTo : FsmStateAction
		{
			[ActionSection("Setup")]
			...
			[ActionSection("General Color Setup")]
			[UIHint(UIHint.FsmColor)]
			public FsmColor color;

			[ActionSection("LineRenderer only Setup")]
			[UIHint(UIHint.FsmColor)]
			[Tooltip("Requires the start colors for the tween, since LineRenderers have no way to get them.  ")]
			public FsmColor startColor1;
			[UIHint(UIHint.FsmColor)]
			public FsmColor startColor2;
			[UIHint(UIHint.FsmColor)]
			public FsmColor endColor1;
			[UIHint(UIHint.FsmColor)]
			public FsmColor endColor2;

		[ActionSection("Material Name Extra")]
		[UIHint(UIHint.FsmString)]
		[Tooltip("Optional -- value of a material(example: _SpecColor, etc")]
		...
		private string debugString = "<b>[DotweenColorRendererTo]</b><color=#E9E581ff> ...mmm did 'DoTween' work ?? </color>";
		...
		public override void Reset ()
		{
				...
				finishEvent = null;
				color = Color.white;
				startColor1= Color.white;
				startColor2= Color.white;
			...
			case loopType.None:
				if (settingLoops > 0 || settingLoops < 0) Debug.LogWarning ("<b>[DotweenColorRendererTo]</b><color=#FF9900ff>!!! Loop Time is set but no 'Loop Type' is selected !!! </color>\",this.Owner");
				break;
			...
			case doTweenType.LineRenderer:
				_target.GetComponent<LineRenderer>().DOColor(new Color2(startColor1.Value,startColor2.Value), new Color2(endColor1.Value, endColor2.Value), duration.Value).SetAs(setFinal);
				break;
				
			case doTweenType.SpriteRenderer:
				_target.GetComponent<SpriteRenderer>().material.DOColor(color.Value,duration.Value).SetAs(setFinal);
				break;
				
			case doTweenType.Material:
				bool aplhaName_isNullOrEmpty = materialName.IsNone || materialName == null || string.IsNullOrEmpty (materialName.Value);
				if (aplhaName_isNullOrEmpty == false)
					_target.GetComponent<Renderer>().material.DOColor(color.Value,materialName.Value,duration.Value).SetAs(setFinal);
				else
				_target.GetComponent<Renderer>().material.DOColor(color.Value,duration.Value).SetAs(setFinal);
				break;

			case doTweenType.Camera:
				_target.GetComponent<Camera>().DOColor(color.Value,duration.Value).SetAs(setFinal);
				break;

			case doTweenType.Light:
				_target.GetComponent<Light>().DOColor(color.Value,duration.Value).SetAs(setFinal);
				break;
