...
namespace HutongGames.PlayMaker.Actions
{
		...
		[Tooltip("Jump using Transform from the target's current position to the given value. ")]
		[HelpUrl("http://dotween.demigiant.com/documentation.php")]
		public class DotweenJumpTransformTo : FsmStateAction
		{
			[ActionSection("Setup")]
			[RequiredField]
		...
		private string debugString = "<b>[DotweenJumpTransformTo]</b><color=#E9E581ff> ...mmm did 'DoTween' work ?? </color>";
		...
		public override void Reset ()
		{
			...
			case loopType.None:
				if (settingLoops > 0 || settingLoops < 0) Debug.LogWarning ("<b>[DotweenJumpTransformTo]</b><color=#FF9900ff>!!! Loop Time is set but no 'Loop Type' is selected !!! </color>",this.Owner);
				break;
			...
			switch(dotweenTypeSelect){
				...
				_target.transform.DOJump(to.Value, jumpPower.Value, jumpNumber.Value, duration.Value,snapping.Value ).SetAs(setFinal);
				break;
					
				case doTweenType.LocalJump:
				_target.transform.DOLocalJump(to.Value, jumpPower.Value, jumpNumber.Value,duration.Value,snapping.Value ).SetAs(setFinal);
					break;
