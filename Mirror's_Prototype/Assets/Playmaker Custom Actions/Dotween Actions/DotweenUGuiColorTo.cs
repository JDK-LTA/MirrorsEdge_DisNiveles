...
namespace HutongGames.PlayMaker.Actions
{
		...
		[Tooltip("Set Color to the renderer from the target's current position to the given value. ")]
		[HelpUrl("http://dotween.demigiant.com/documentation.php")]
		public class DotweenUGuiColorTo : FsmStateAction
		{
			[ActionSection("Setup")]
			[RequiredField]
		...
		private string debugString = "<b>[DotweenUGuiColorTo]</b><color=#E9E581ff> ...mmm did 'DoTween' work ?? </color>";
		...
		public override void Reset ()
		{
			...
			case loopType.None:
				if (settingLoops > 0 || settingLoops < 0) Debug.LogWarning ("<b>[DotweenUGuiColorTo]</b><color=#FF9900ff>!!! Loop Time is set but no 'Loop Type' is selected !!! </color>",this.Owner);
				break;
			...
			case doTweenType.Graphic:
				_target.GetComponent<UnityEngine.UI.Graphic>().DOColor(color.Value,duration.Value).SetAs(setFinal);
				break;
				
			case doTweenType.Image:
				_target.GetComponent<UnityEngine.UI.Image>().DOColor(color.Value,duration.Value).SetAs(setFinal);
				break;
				
			case doTweenType.Outline:
				_target.GetComponent<UnityEngine.UI.Outline>().DOColor(color.Value,duration.Value).SetAs(setFinal);
				break;

			case doTweenType.Text:
				_target.GetComponent<UnityEngine.UI.Text>().DOColor(color.Value,duration.Value).SetAs(setFinal);
				break;
