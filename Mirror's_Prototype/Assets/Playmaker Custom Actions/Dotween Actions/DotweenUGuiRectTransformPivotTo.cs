...
namespace HutongGames.PlayMaker.Actions
{
		...
		[Tooltip("Tweens the target's pivot")]
		[HelpUrl("http://dotween.demigiant.com/documentation.php")]
		public class DotweenUGuiRectTransformPivotTo : FsmStateAction
		{
			[ActionSection("Setup")]
			[RequiredField]
		...
		private string debugString = "<b>[DotweenUGuiRectTransformPivotTo]</b><color=#E9E581ff> ...mmm did 'DoTween' work ?? </color>";
		...
		public override void Reset ()
		{
			...
			case loopType.None:
				if (settingLoops > 0 || settingLoops < 0) Debug.LogWarning ("<b>[DotweenUGuiRectTransformPivotTo]</b><color=#FF9900ff>!!! Loop Time is set but no 'Loop Type' is selected !!! </color>",this.Owner);
				break;
			...
			case doTweenType.AnchoredPivot2D:
				_target.GetComponent<RectTransform>().DOAnchorPos(Vector2setup,duration.Value).SetAs(setFinal);
				break;
	
			case doTweenType.AnchoredPivotX:
				_target.GetComponent<RectTransform>().DOAnchorPosX(Vector2setup.x,duration.Value).SetAs(setFinal);
				break;

			case doTweenType.AnchoredPivotY:
				_target.GetComponent<RectTransform>().DOAnchorPosY(Vector2setup.y,duration.Value).SetAs(setFinal);
				break;
			}
