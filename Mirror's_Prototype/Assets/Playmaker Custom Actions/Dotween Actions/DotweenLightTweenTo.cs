...
namespace HutongGames.PlayMaker.Actions
{
		...
		[Tooltip("Set Light intensity or Set Shadow Strength. ")]
		[HelpUrl("http://dotween.demigiant.com/documentation.php")]
		public class DotweenLightTweenTo : FsmStateAction
		{
			[ActionSection("Setup")]
		...
		private string debugString = "<b>[DotweenLightTweenTo]</b><color=#E9E581ff> ...mmm did 'DoTween' work ?? </color>";
		...
		public override void Reset ()
		{
			...
			case loopType.None:
				if (settingLoops > 0 || settingLoops < 0) Debug.LogWarning ("<b>[DotweenLightTweenTo]</b><color=#FF9900ff>!!! Loop Time is set but no 'Loop Type' is selected !!! </color>",this.Owner);
				break;
			...
			case doTweenType.Intensity:
				_target.GetComponent<Light>().DOIntensity(tweenTo.Value, duration.Value).SetAs(setFinal);
				break;
				
			case doTweenType.ShadowStrength:
				_target.GetComponent<Light>().DOShadowStrength(tweenTo.Value,duration.Value).SetAs(setFinal);
				break;
