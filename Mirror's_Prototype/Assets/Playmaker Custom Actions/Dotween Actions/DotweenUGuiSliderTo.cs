...
namespace HutongGames.PlayMaker.Actions
{
		...
		[Tooltip("Changes the target's Slider value to the given one. ")]
		[HelpUrl("http://dotween.demigiant.com/documentation.php")]
		public class DotweenUGuiSliderTo : FsmStateAction
		{
			[ActionSection("Setup")]
			[RequiredField]
			...
			[ActionSection("Slide Setup")]
			[UIHint(UIHint.FsmFloat)]
			public FsmFloat setSlideTo;
		...
		private string debugString = "<b>[DotweenUGuiSliderTo]</b><color=#E9E581ff> ...mmm did 'DoTween' work ?? </color>";
		...
		public override void Reset ()
		{
			...
			if (loopDontFinish.Value == true) settingLoops.Value = -1;
			else settingLoops.Value = loopsQuantity.Value;
			...
			case loopType.None:
				if (settingLoops.Value > 0 || settingLoops.Value < 0) Debug.LogWarning ("<b>[DotweenUGuiSliderTo]</b><color=#FF9900ff>!!! Loop Time is set but no 'Loop Type' is selected !!! </color>",this.Owner);
				break;

			case loopType.Yoyo:
				setFinal.SetLoops(settingLoops.Value, LoopType.Yoyo);
				break;

			case loopType.Restart:
				setFinal.SetLoops(settingLoops.Value, LoopType.Restart);
				break;

			case loopType.Incremental:
				setFinal.SetLoops(settingLoops.Value, LoopType.Incremental);
				break;
			...
			_target.GetComponent<UnityEngine.UI.Slider>().DOValue(setSlideTo.Value, duration.Value, snapping.Value).SetAs(setFinal);
