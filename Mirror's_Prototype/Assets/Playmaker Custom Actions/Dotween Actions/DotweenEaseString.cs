...
namespace HutongGames.PlayMaker.Actions
{
		...
		public class DotweenEaseString : FsmStateAction
		{
		
			[ActionSection("Setup")]
			[UIHint(UIHint.FsmString)]
			public FsmString fromValue;
			[UIHint(UIHint.FsmString)]
			public FsmString toValue;
			[UIHint(UIHint.FsmString)]
			public FsmString resultValue;

			[ActionSection("Options")]
			[UIHint(UIHint.FsmBool)]
			public FsmBool richTextOn;
		...
		public scrambleType scrambleTypeSelect;
		[UIHint(UIHint.FsmString)]
		[Tooltip("A string containing the characters to use for custom scrambling. Use as many characters as possible (minimum 10)")]	 
		...
		ScrambleMode scrambleTypeSelected;

		private string debugString = "<b>[DotweenEaseString]</b><color=#E9E581ff> ...mmm did 'DoTween' work ?? </color>";
			
		public override void Reset ()
		{
				toValue = new FsmString { UseVariable = true };;
				fromValue = new FsmString { UseVariable = true };;
				resultValue = new FsmString { UseVariable = true };;
				duration = 0f;
				setDelay = 0f;
				isSpeedBased = false;
				settingLoops = 0;
			...
			if (scrambleTypeSelect != scrambleType.None && scrambleChars.Value.Length > 0 && scrambleChars.Value.Length <10) Debug.LogWarning ("<b>[DotweenEaseString]</b><color=#FF9900ff!!!  Use as many characters as possible (minimum 10) in 'Scramble Chars' because DOTween uses a fast scramble mode which gives better results with more characters. !!! </color>",this.Owner);


			switch(loopTypeSelect){
			case loopType.None:
				if (settingLoops > 0 || settingLoops < 0) Debug.LogWarning ("<b>[DotweenEaseString]</b><color=#FF9900ff>!!! Loop Time is set but no 'Loop Type' is selected !!! </color>",this.Owner);
				break;
