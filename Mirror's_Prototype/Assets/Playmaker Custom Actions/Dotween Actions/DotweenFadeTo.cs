...
namespace HutongGames.PlayMaker.Actions
{
		...
		[Tooltip("Fade using the renderer or material or audio from the target's current position to the given value. ")]
		[HelpUrl("http://dotween.demigiant.com/documentation.php")]
		public class DotweenFadeTo : FsmStateAction
		{
			[ActionSection("Setup")]
			...
			[ActionSection("Fade Setup")]
			[UIHint(UIHint.FsmFloat)]
			[Tooltip("Alpha or Volume NOTE: works only with materials that support alpha")]	
			[TitleAttribute("Fade Alpha/Volume to")]
		...
		[ActionSection("Material Fade Extra")]
		[UIHint(UIHint.FsmString)]
		[Tooltip("Optional -- Fade to target's named alpha property(example: _TintColor, _Color, etc")]
		public FsmString alphaName;
		...
		private string debugString = "<b>[DotweenFadeTo]</b><color=#E9E581ff> ...mmm did 'DoTween' work ?? </color>";
		...
		public override void Reset ()
		{
			...
			case loopType.None:
				if (settingLoops > 0 || settingLoops < 0) Debug.LogWarning ("<b>[DotweenFadeTo]</b><color=#FF9900ff>!!! Loop Time is set but no 'Loop Type' is selected !!! </color>",this.Owner);
				break;
			...
			case doTweenType.AudioSource:
				_target.GetComponent<AudioSource>().DOFade(fadeTo.Value, duration.Value).SetAs(setFinal);
				break;
				
			case doTweenType.Material:
				bool aplhaName_isNullOrEmpty = alphaName.IsNone || alphaName == null || string.IsNullOrEmpty (alphaName.Value);
				if (aplhaName_isNullOrEmpty == false)
					_target.GetComponent<Renderer>().material.DOFade(fadeTo.Value,alphaName.Value, duration.Value).SetAs(setFinal);
					else
				_target.GetComponent<Renderer>().material.DOFade(fadeTo.Value,duration.Value).SetAs(setFinal);
				break;
				
			case doTweenType.SpriteRenderer:
				_target.GetComponent<SpriteRenderer>().material.DOFade(fadeTo.Value,duration.Value).SetAs(setFinal);
				break;
