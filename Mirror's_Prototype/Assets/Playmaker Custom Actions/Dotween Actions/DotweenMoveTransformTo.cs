...
namespace HutongGames.PlayMaker.Actions
{
		...
		[Tooltip("Moves using Transform from the target's current position to the given value. ")]
		[HelpUrl("http://dotween.demigiant.com/documentation.php")]
		public class DotweenMoveTransformTo : FsmStateAction
		{
			[ActionSection("Setup")]
			[RequiredField]
		...
		private string debugString = "<b>[DotweenMoveTransformTo]</b><color=#E9E581ff> ...mmm did 'DoTween' work ?? </color>";
		...
		public override void Reset ()
		{
			...
			case loopType.None:
				if (settingLoops > 0 || settingLoops < 0) Debug.LogWarning ("<b>[DotweenMoveTransformTo]</b><color=#FF9900ff>!!! Loop Time is set but no 'Loop Type' is selected !!! </color>",this.Owner);
				break;
			...
			switch(dotweenTypeSelect){
				...
				_target.transform.DOMove(to.Value, duration.Value,snapping.Value ).SetAs(setFinal);
				break;
					
				case doTweenType.DoMoveX:
					float x = to.Value.x;
				_target.transform.DOMoveX(x, duration.Value,snapping.Value ).SetAs(setFinal);
					break;
					
				case doTweenType.DoMoveY:
					float y = to.Value.y;
				_target.transform.DOMoveY(y, duration.Value,snapping.Value ).SetAs(setFinal);
					break;
					
				case doTweenType.DoMoveZ:
					float z = to.Value.z;
				_target.transform.DOMoveZ(z, duration.Value,snapping.Value ).SetAs(setFinal);
					break;
					
				case doTweenType.DoLocalMove:
				_target.transform.DOLocalMove(to.Value, duration.Value,snapping.Value ).SetAs(setFinal);
					break;
					
				case doTweenType.DoLocalMoveX:
					float xx = to.Value.x;
				_target.transform.DOLocalMoveX(xx, duration.Value,snapping.Value ).SetAs(setFinal);
					break;
					
				case doTweenType.DoLocalMoveY:
					float yy = to.Value.y;
				_target.transform.DOLocalMoveY(yy, duration.Value,snapping.Value ).SetAs(setFinal);
					break;
					
				case doTweenType.DoLocalMoveZ:
					float zz = to.Value.z;
				_target.transform.DOLocalMoveZ(zz, duration.Value,snapping.Value ).SetAs(setFinal);
					break;
