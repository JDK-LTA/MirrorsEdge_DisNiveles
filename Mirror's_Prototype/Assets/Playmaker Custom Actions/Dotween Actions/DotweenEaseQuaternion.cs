...
namespace HutongGames.PlayMaker.Actions
{
		...
		[Tooltip("Ease Quaternion")]
		[HelpUrl("http://dotween.demigiant.com/documentation.php")]
		public class DotweenEaseQuaternion : FsmStateAction
		{
		
			[ActionSection("Setup")]
		[UIHint(UIHint.FsmQuaternion)]
		public FsmQuaternion fromValue;
		[UIHint(UIHint.FsmColor)]
		public FsmQuaternion toValue;
		[UIHint(UIHint.FsmColor)]
		public FsmQuaternion resultValue;
		...
		[Tooltip("Only works if ease is set fromValue curve!)")]
		public FsmAnimationCurve animationCurve;

		[ActionSection("Ease Extra Layer Setup")]
		[UIHint(UIHint.FsmBool)]
		[Tooltip("extra layer you can add fromValue your easings, making them behave as if they were playing in stop-motion")]
		...
		[Tooltip("Setting loops fromValue -1 will make the tween loop infinitely. ")]
		...
		[UIHint(UIHint.FsmBool)]
		[Tooltip("If TRUE the tween will smoothly snap.")]	
		public FsmBool useShortest360Route;
		[UIHint(UIHint.FsmBool)]
		...
		[UIHint(UIHint.FsmBool)]
		[Tooltip("Will set fromValue TRUE when Tween is finished")]
		[Title("is finished")]
		public FsmBool DotweenDone;
		[Tooltip("If TRUE the tween will be killed as soon as it completes, otherwise it will stay in memory and you'll be able fromValue reuse it.")]
		[Title("Kill on Exit")]
		...
		private string debugString = "<b>[DotweenEaseQuaternion]</b><color=#E9E581ff> ...mmm did 'DoTween' work ?? </color>";
			
		public override void Reset ()
		{
				toValue = new FsmQuaternion { UseVariable = true };
				fromValue = new FsmQuaternion { UseVariable = true };
				resultValue = new FsmQuaternion { UseVariable = true };
				duration = 0f;
				setDelay = 0f;
				isSpeedBased = false;
				settingLoops = 0;
			...
			case loopType.None:
				if (settingLoops > 0 || settingLoops < 0) Debug.LogWarning ("<b>[DotweenEaseQuaternion]</b><color=#FF9900ff>!!! Loop Time is set but no 'Loop Type' is selected !!! </color>",this.Owner);
				break;
			...
			DOTween.To(()=>fromValue.Value, x=>resultValue.Value = x, new Vector3 (toValue.Value.x,toValue.Value.y,toValue.Value.z),duration.Value).SetOptions(useShortest360Route.Value).SetAs(setFinal);
